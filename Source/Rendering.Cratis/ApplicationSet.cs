...
public class ApplicationSet
{
    ...
    /// <summary>
    /// Gets every policy declared across the applications, keyed by name. An <c language="csharp">authorize</c> names policies rather
    ...
    /// <summary>
    /// Gets the short names of every <c language="csharp">import</c> declared across the applications — constructs owned by another
    ...
    /// <summary>
    /// Gets the names of every concept used by at least one command property marked <c language="csharp">identifier</c> — these
    /// concepts render as <c language="csharp">EventSourceId&lt;T&gt;</c> rather than <c language="csharp">ConceptAs&lt;T&gt;</c>.
    /// </summary>
    ...
    /// property marked <c language="csharp">identifier</c> is one; so is a concept a projection uses as its <c language="csharp">key</c>, since a read
    /// model's key is the event source it is built from — rendering it as a plain <c language="csharp">ConceptAs&lt;T&gt;</c> leaves
    /// it with no conversion to <c language="csharp">EventSourceId</c>, which every generated by-id query needs.
    /// </summary>
