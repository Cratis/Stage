...
public class with_a_localized_requirement_message : given.an_invoice_model
{
    void Because() => Plan(Invoices.Replace(
        "        produces InvoiceIssued\n",
        "        validate\n          require description == \"First payload\"\n            message $strings.invoice.descriptionRequired\n        produces InvoiceIssued\n",
        StringComparison.Ordinal));

    [Fact] void should_not_plan_any_artifacts() => _plan.Artifacts.ShouldBeEmpty();
    [Fact] void should_reject_the_unresolved_string_key() => ErrorCodes.ShouldContainOnly(["STAGE-ESM-005"]);
}
