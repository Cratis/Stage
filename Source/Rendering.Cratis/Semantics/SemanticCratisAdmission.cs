...
internal static partial class SemanticCratisAdmission
{
        ...
        IReadOnlyList<LocatedSemanticSlice> slices)
    {
        ...
        foreach (var located in slices)
        {
            ...
            {
                diagnostics.Add(Error("STAGE-ESM-019", $"Slice '{located.Slice.Name}' has reducer implementation bodies; typed contexts are available, but Stage#119 still needs to render reducer bodies with null-result deletion, event-source key semantics and pure capability enforcement; ESM v4 admission is also pending.", located.Slice.Id));
                continue;
            }
