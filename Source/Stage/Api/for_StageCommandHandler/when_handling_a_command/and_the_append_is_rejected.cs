...
public class and_the_append_is_rejected : given.a_command_handler
{
    ...
    async Task Because()
    {
        _appender.Append(Arg.Any<string>(), Arg.Any<IReadOnlyList<Runtime.ProducedEventPayload>>(), Arg.Any<IReadOnlyDictionary<string, string>>())
            .Returns(Task.FromResult<CommandResult?>(CommandResult.Error(Cratis.Execution.CorrelationId.New(), "Append rejected")));
